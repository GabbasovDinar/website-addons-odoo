...
from openerp.addons.base.ir.ir_config_parameter import ir_config_parameter as IrConfigParameterOriginal, _default_parameters
...
class IrConfigParameter(models.Model):
    ...
    def _auto_init(self, cr, context=None):
        ...
        return super(IrConfigParameter, self)._auto_init(cr, context=context)

    def _auto_end(self, cr, context=None):
        super(IrConfigParameter, self)._auto_end(cr, context=context)

        # rename "value_tmp" back to "value_tmp"
        cr.execute("ALTER TABLE ir_config_parameter RENAME COLUMN value_tmp TO value")

        config_ids = self.pool['ir.config_parameter'].search(cr, SUPERUSER_ID, [])
        for r in self.pool['ir.config_parameter'].browse(cr, SUPERUSER_ID, config_ids, context=context):
